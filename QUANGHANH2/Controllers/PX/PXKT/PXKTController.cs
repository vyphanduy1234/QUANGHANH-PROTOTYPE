        ...
        public dynamic getAll(int session, string departmentID, DateTime date)
        {
            using (QUANGHANHABCEntities db = new QUANGHANHABCEntities())
            {
                ...
                var query = "select b.DiLam as [status], b.HeaderID as headerID,nv.MaNV,nv.Ten as tenNV,b.ThoiGianThucTeDiemDanh as timeAttendance,b.reason,[b].[description] " +
                    ...
                    "right join(select* from NhanVien where MaPhongBan = @departmentID and (MaCongViec = 34 or MaCongViec = 33 or MaCongViec = 60 or MaCongViec = 72 or MaCongViec = 73)) as nv on b.MaNV = nv.MaNV";
